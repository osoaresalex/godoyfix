# IMPORTANTE: AÇÃO NECESSÁRIA ANTES DA PRODUÇÃO

**Os Deep Links do aplicativo (Android e iOS) estão atualmente configurados para `localhost` para permitir testes locais. Antes de gerar uma versão de produção, você DEVE substituir `localhost` pelo seu domínio real.**

*   **Android:** Altere o `android:host` no arquivo `android/app/src/main/AndroidManifest.xml`.
*   **iOS:** Altere o valor `applinks` no arquivo `ios/Runner/Runner.entitlements`.
