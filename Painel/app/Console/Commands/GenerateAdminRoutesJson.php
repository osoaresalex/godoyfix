        ...
        if (File::exists($modulesBasePath)) {
            ...
            foreach ($modules as $modulePath) {
                ...
                if (File::exists($moduleViewsPath)) {
                    $moduleViewPath = $moduleViewsPath . '/' . $viewPath . '.blade.php';
                    if (File::exists($moduleViewPath)) {
                        return $moduleViewPath;
                    }
                    $modulePhpPath = $moduleViewsPath . '/' . $viewPath . '.php';
                    if (File::exists($modulePhpPath)) {
                        return $modulePhpPath;
                    }
                    $moduleDirPath = $moduleViewsPath . '/' . $viewPath;
                ...
                if (File::exists($altModuleViewsPath)) {
                    $altModuleViewPath = $altModuleViewsPath . '/' . $viewPath . '.blade.php';
        ...
        if (function_exists('module_path')) {
            try {
                if (strpos($viewPath, '::') !== false) {
                    ...
                    $moduleViewPath = module_path($moduleName, 'Resources/views/' . $viewName . '.blade.php');
                    if (File::exists($moduleViewPath)) {
                        return $moduleViewPath;
                    }
                }
    ...
    private function generateAndSaveJsonFiles($items,$type): void
    {
        ...
        $langItems = $filteredItems->map(function ($item) {
            ...
            foreach ($keywords as $keyword) {
                $processedKey = ucfirst(str_replace('_', ' ', $this->removeSpecialCharacters($keyword)));
