            ...
            @can('configuration_manage_status')
                ...
                <div class="bg-warning bg-opacity-10 fs-12 p-12 text-dark rounded mt-3 mb-3">
                    <div class="d-flex align-items-center gap-2 mb-2">
                        ...
                        <p class="fz-12">{{ translate('Setup Push Notification Messages for customers.') }} {{ translate('Must setup') }} <a href="{{ route('admin.configuration.get-notification-setting', ['type' => 'customers']) }}" class="text-primary text-decoration-underline fw-medium">{{ translate('Firebase Configuration') }}</a>  {{ translate('to work notifications.') }}</p>
                    </div>
                    <ul class="m-0 ps-20 d-flex flex-column gap-1 text-dark">
                        <li>{{ translate('Push notifications will work properly once the') }} <a href="{{ route('admin.configuration.third-party', 'firebase-configuration') }}" class="fw-medium text-primary text-decoration-underline">{{ translate('Firebase configure') }}</a> {{ translate('setup properly') }}</li>
                        <li>{{ translate('Setup') }} <a href="{{ route('admin.configuration.third-party', 'email-config') }}" class="fw-medium text-primary text-decoration-underline">{{ translate('Mail Configuration') }}</a> {{ translate('to work mail properly') }}</li>
                        <li>{{ translate('At least one') }} <a href="{{ route('admin.configuration.third-party', 'sms_config') }}" class="fw-medium text-primary text-decoration-underline">{{ translate('SMS Method') }} </a>{{ translate('must be enable to Work SMS properly') }}</li>
                    </ul>
                </div>
