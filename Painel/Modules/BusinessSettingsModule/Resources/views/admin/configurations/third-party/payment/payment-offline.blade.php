<div class="tab-pane fade {{ request()->has('type') && request()->type == 'offline_payment' ? 'show active' : '' }}" id="payment-tabs2" role="tabpanel" aria-labelledby="payment-custom-tab2" tabindex="0">
    <div class="bg-warning bg-opacity-10 fs-12 p-12 text-dark rounded mb-10">
        ...
        <ul class="m-0 ps-20 d-flex flex-column gap-1 text-dark">
            ...
            <li>{{ translate('To make available these payment options, you must enable the Offline payment option from ') }}<a href="{{ route('admin.business-settings.get-business-information') }}" class="fw-semibold text-primary text-decoration-underline" target="_blank">{{ translate('Business Information') }}</a> {{ translate('page.') }} </li>
        </ul>
    </div>
    <div class="card">
        <div class="card-body">
