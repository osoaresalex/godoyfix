<div class="tab-pane fade {{ request()->has('type') && request()->type == 'digital_payment' ? 'show active' : '' }}" id="payment-tabs1" role="tabpanel" aria-labelledby="payment-custom-tab1" tabindex="0">
    <div class="bg-warning bg-opacity-10 fs-12 p-12 text-dark rounded mb-10">
        ...
        <ul class="m-0 ps-20 d-flex flex-column gap-1 text-dark">
            ...
            <li>{{ translate('To make available these payment options, you must enable the Digital payment option from ') }}<a href="{{ route('admin.business-settings.get-business-information') }}" class="fw-semibold text-primary text-decoration-underline" target="_blank">{{ translate('Business Information') }}</a> {{ translate('_page.') }}</li>
        </ul>
    </div>
    <?php
        $currencySupported = 0;
