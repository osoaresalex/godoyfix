@if($webPage == 'storage_connection')
    <div class="tab-content">
        @php
            ...
            $isS3FieldEmpty = is_array($credentials) && count(array_filter($credentials, fn($v) => !empty($v))) === count($credentials);
        @endphp
        <div class="tab-pane fade {{$webPage == 'storage_connection' ? 'show active' : ''}}" id="storage_connection">
            <div class="pick-map mb-3 p-12 rounded d-flex flex-md-nowrap flex-wrap align-items-center gap-1 bg-primary bg-opacity-10">
                ...
                <p class="fz-12">{{ translate('You can manage all your storage files from') }}  <a href="{{ route('admin.business-settings.get-gallery-setup') }}" target="_blank" class="text-primary fw-semibold text-decoration-underline"> {{ translate('Gallery') }}</a></p>
            </div>
                                ...
                                <div class="bg-white rounded-2 p-16">
                                    <div class="row g-xl-4 g-3">
                                        ...
                                        <div class="col-md-6" @if($isS3FieldEmpty)
                                            data-bs-toggle="tooltip"
                                             data-bs-placement="top"
                                             title="{{translate('3rd_party_storage_is_currently_disabled_please_configure_following_data_first')}}"
                                        @endif>
