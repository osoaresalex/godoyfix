...
class ConfigurationController extends Controller
{
    ...
    public function thirdPartyConfigGet(Request $request): View|Factory|Application
    {
        ...
        if ($webPage == 'sms_config') {
            ...
            // Ensure all digital gateways have addon_settings rows so they appear in the list (including new ones like mercadopago_pix)
            $digitalGatewayKeys = array_column(DIGITAL_PAYMENT_METHODS, 'key');
            foreach ($digitalGatewayKeys as $gatewayKey) {
                $exists = $this->addonSetting
                    ->where('settings_type', 'payment_config')
                    ->where('key_name', $gatewayKey)
                    ->exists();

                if (!$exists) {
                    // Default credential fields per gateway for proper form rendering
                    $defaultFields = [];
                    switch ($gatewayKey) {
                        case 'ssl_commerz':
                            $defaultFields = ['store_id' => '', 'store_password' => ''];
                            break;
                        case 'stripe':
                            $defaultFields = ['api_key' => '', 'published_key' => ''];
                            break;
                        case 'razor_pay':
                            $defaultFields = ['api_key' => '', 'api_secret' => ''];
                            break;
                        case 'senang_pay':
                            $defaultFields = ['callback_url' => '', 'secret_key' => '', 'merchant_id' => ''];
                            break;
                        case 'paystack':
                            $defaultFields = ['public_key' => '', 'secret_key' => '', 'merchant_email' => ''];
                            break;
                        case 'flutterwave':
                            $defaultFields = ['secret_key' => '', 'public_key' => '', 'hash' => ''];
                            break;
                        case 'paytm':
                            $defaultFields = ['merchant_key' => '', 'merchant_id' => '', 'merchant_website_link' => ''];
                            break;
                        case 'mercadopago_pix':
                            $defaultFields = ['access_token' => ''];
                            break;
                        default:
                            $defaultFields = [];
                    }
                    ...
                    $gatewayTitle = collect(DIGITAL_PAYMENT_METHODS)
                        ->firstWhere('key', $gatewayKey)['value'] ?? ucfirst(str_replace('_', ' ', $gatewayKey));

                    $this->addonSetting->updateOrCreate(
                        ['key_name' => $gatewayKey, 'settings_type' => 'payment_config'],
                        [
                            'key_name' => $gatewayKey,
            ...
            // Load only digital payment gateways for the UI list
            $dataValues = $this->addonSetting
                ->where('settings_type', 'payment_config')
                ->whereIn('key_name', $digitalGatewayKeys)
                ->get();
...
//            $desiredName = $webPage === 'sms_config' ? 'sms_setup' : 'payment_setup';
//
//            $paymentUrl = collect($routes)
//                ->flatMap(fn($group) => $group)
//                ->firstWhere('name', $desiredName)['url'] ?? '';
            ...
            // Garantir que o PIX esteja incluído mesmo que alguma instalação não tenha atualizado DIGITAL_PAYMENT_METHODS
            if ($webPage === 'payment_config') {
                if (!in_array('mercadopago_pix', $keyNames, true)) {
                    $keyNames[] = 'mercadopago_pix';
                }
                $keyNames = array_values(array_unique($keyNames));
            }

            // Seed missing digital payment gateways (including mercadopago_pix) so they appear in the list
            if ($webPage === 'payment_config' && ($type === 'digital_payment' || is_null($type))) {
                foreach ($keyNames as $gatewayKey) {
                    $exists = $this->addonSetting
                        ->where('settings_type', 'payment_config')
                        ->where('key_name', $gatewayKey)
                        ->exists();
...
//            'app_settings' => $data = array_merge(['customer_app_settings' => json_decode($this->getThirdPartyData(webPage: 'customer_app_settings'), true)], ['provider_app_settings' => json_decode($this->getThirdPartyData(webPage: 'provider_app_settings'), true)], ['serviceman_app_settings' => json_decode($this->getThirdPartyData(webPage: 'serviceman_app_settings'), true)]),
