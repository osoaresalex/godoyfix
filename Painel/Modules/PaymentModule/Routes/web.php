...
use Modules\PaymentModule\Http\Controllers\MercadoPagoPixController;
...
if (!$isPublished) {
    Route::group(['prefix' => 'payment'], function () {
        ...
        Route::group(['prefix' => 'mercadopago_pix', 'as' => 'mercadopago_pix.'], function () {
            ...
            Route::post('webhook', [MercadoPagoPixController::class, 'webhook'])
                ->name('webhook')
                ->withoutMiddleware([VerifyCsrfToken::class]);
            Route::get('check-status', [MercadoPagoPixController::class, 'checkStatus'])->name('check-status');
            Route::get('redirect-success', [MercadoPagoPixController::class, 'redirectSuccess'])->name('redirect-success');
        });
    });
}
