    ...
    @if (count($result) > 0)
        ...
        @foreach ($result as $groupKey => $routes)
            ...
            @endphp
                ...
                <form action="{{ route('admin.search.routing.store') }}" method="POST" class="w-100 d-block">
                    @csrf
                    ...
                    <button type="submit"
                        ...
                        <span class="d-flex flex-column me-3 flex-grow-1">
                            <span class="fw-medium text-dark" style="font-size:14px;">{!! $highlightedTitle !!}</span>
                            <span class="text-muted mt-3 d-inline-block"
