...
import 'package:demandium/utils/label_mapper.dart';
  ...
  Future<void> getZoneList({Map<String, GlobalKey>? globalKeyMap, bool reload = true}) async {
    ...
    DataSyncHelper.fetchAndSyncData(
      ...
      onResponse: (data, source) {
        ...
        // Check if response has 'data' array or 'zone' object
        if (data['content'] != null) {
          if (data['content']['data'] is List) {
            data['content']['data'].forEach((zone) => _zoneList!.add(ZoneModel.fromJson(zone)));
          } else if (data['content']['zone'] != null) {
            _zoneList!.add(ZoneModel.fromJson(data['content']['zone']));
          }
        }
    ...
    for (int index = 0; index < zoneList.length; index++) {
      ...
      markerList.add(Marker(
        infoWindow: GetPlatform.isWeb || GetPlatform.isIOS ? InfoWindow(
            title: localizeZoneName(zoneList[index].name)
        ) : InfoWindow.noText,
